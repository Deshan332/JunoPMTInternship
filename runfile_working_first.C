		...
		KST->GetYaxis()->SetTitle("Counts");
	 	...
	 	{	
				...
				if (SPEM == 0) {gaindata <<"angle Mu Mu_err w w_err alpha alpha_err lambda lambda_err Theta Theta_err sig_reduced sig_reduced_err Gain Gain_err chi_sq chi_sub fit.status D  gainerror/Gfit is Point Good? "<< endl;}
	 			if (SPEM == 1) {gaindata <<"angle Mu Mu_err w w_err alpha alpha_err Q Q_err s s_err sig_reduced sig_reduced_err Gain Gain_err chi_sq chi_sub fit.status D  gainerror/Gfit is Point Good? "<< endl;}
				gaindata <<" "<< endl;
				...
				Int_t count = 0;
	 			...
	 			for (int a=0; a<24; ++a)
					...
					if(SPEM == 1)
					{
						...
						grBF->Draw( "SAME,L" );	grBF2->Draw( "SAME,L" ); 	c1->Update(); c1->WaitPrimitive(); c1->Print(PdfName_mid ,"pdf"); 
						...
						if(SPEM == 0)	{
							if ((fit.chi2r <= 3) && (fit.fit_status == 0)  && (gainerror/Gfit<0.08) )	{ANGLES[count] = 15*a;  PMT_DATA[2*p-2][count] = Gfit;	PMT_DATA[2*p-1][count] = gainerror;   ++count;	STATUS = "Yes";}
						else {STATUS = "No"; ++REM;}
								}
						if(SPEM == 1)	{
							if ((fit.chi2r <= 3) && (fit.fit_status == 0) && (gainerror/Gfit<0.09) )	{ANGLES[count] = 15*a;  PMT_DATA[2*p-2][count] = Gfit;	PMT_DATA[2*p-1][count] = gainerror;   ++count;	STATUS = "Yes";}
						else {STATUS = "No"; ++REM;}
								}
						...
						<<Gfit <<"  "<< gainerror<<"  "<< fit.chi2r<<"  "<<chi_red<<"  "<<fit.fit_status<<"  "<<D<<"  "<<gainerror/Gfit<<"  "<<STATUS<<"  "<<endl;
			...
			auto gr_5 = new TGraphErrors(counts[4],ANGLES_5,PMT_DATA[8],X_ERR,PMT_DATA[9]);	gr_5->SetMarkerColor(93); gr_5->SetLineColor(93); gr_5->SetMarkerStyle(7); gr_5->SetName("Position 5"); gr_5->SetTitle("Position 5");
			...
			auto gr2_5 = new TGraphErrors(counts[4],ANGLES_5,PMT_DATA_NORM[8],X_ERR,PMT_DATA_NORM[9]);	gr2_5->SetMarkerColor(93); gr2_5->SetLineColor(93); gr2_5->SetMarkerStyle(7); gr2_5->SetName("Position 5"); gr2_5->SetTitle("Position 5");
